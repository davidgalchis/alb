    ...
    try:
        ...
        if response and response.get("TargetGroups") and len(response.get("TargetGroups")) > 0:
            ...
            prev_targets_to_remove = [remove_none_attributes({ \
                "Id": prev_target.split("$")[0] or None, \
                "Port": safe_cast(prev_target.split("$")[1], int, prev_target.split("$")[1]) or None, \
                "AvailabilityZone": prev_target.split("$")[2] or None \
                }) for prev_target in prev_targets_comparable if (prev_target not in targets_comparable) and (prev_target != "$$")]
            targets_to_add = [remove_none_attributes({ \
                ...
                }) for def_target in targets_comparable if (def_target not in prev_targets_comparable) and (def_target != "$$")]
